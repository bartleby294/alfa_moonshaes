...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
/*
Modified Version 1.2 (UC)
Modified By:    U'lias Clearmoon (Shawn Marcil)
Last Modified:  October 5, 2004

Modification Description:
-   Added new functions "UC_GetDestinationArea" and "UC_GetCurrentAreaWPTag"
    so that the script is dependent on waypoint tags rather than area tags,
    so area tags are freed up for other purposes.
-   Add a single waypoint to each area, and give them a tag as you would
    have for an area using the previous version of this system. Ensure that
    each AT waypoint's name is the exact same as the tag except add the
    prefix of "AT_" to it.

*/
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
const int UC_USE_WAYPOINTS = TRUE; // make this FALSE to use default Area Tags for determining destination areas
const string UC_AT_WP_PREFIX = "AT_";
const int nDEBUG = FALSE;             // Turns on feedback while zoning.  Use for problem solving only, normal state is FALSE.\
const float UC_INVIS_DURATION = 12.0f; // determines how long an object remains invis
const float fTransitionDelay = 2.0f;  // Delay before zoning, used to prevent cheating by PC's (2.0 to 4.0 recommended)
...
object UC_GetDestinationArea ( object oPC , string sCoordsTag );
string UC_GetCurrentAreaWPTag ( object oPC );
...
{ /// Begin User-Defined Variables ///
  ...
  /// End User-Defined Variables   ///
    ...
    if ((oPC != OBJECT_INVALID) && (GetLocalInt(oPC,"m_nZoning") != TRUE))
    {
        ...
        effect eZoneEffect = EffectVisualEffect(VFX_IMP_ACID_L);   // ZoneIn effect for end-of-transition
...
/*modified by UC*/
        string sCurAreaTag;         // X,Y,Z Tag of Current Area
        string sXYTag;              // Separate out X/Y from Z coordinate
        string sCurAreaX;           // X-coordinate of current area (from Tag)
        string sCurAreaY;           // Y-coordinate of current area (from Tag)
        string sCurAreaZ;           // Z-coordinate of current area (from Tag)

/*added by UC*/
        if (UC_USE_WAYPOINTS) {
            sCurAreaTag  = GetStringLowerCase(UC_GetCurrentAreaWPTag(oPC));  // X,Y,Z Tag of Current Area
/*default variable assignment*/
        } else {
            sCurAreaTag  = GetStringLowerCase(GetTag(GetArea(oPC)));         // X,Y,Z Tag of Current Area
        }

        sXYTag       = GetStringLeft(sCurAreaTag,6);                     // Separate out X/Y from Z coordinate
...
/*added by UC*/ if (UC_USE_WAYPOINTS) {
                    oArea = UC_GetDestinationArea(oPC, (sCurAreaX + sDestAreaY + sCurAreaZ));
                } else {
                ...
                if ((oArea != OBJECT_INVALID) && (GetAreaFromLocation(lLoc) == oArea))
                {
                    ...
                    DelayCommand(fTransitionDelay,ApplyEffectAtLocation(DURATION_TYPE_INSTANT,eZoneEffect,lLoc));
                }
                break;
            }
            case 2:  // Moving SOUTH
...
/*added by UC*/ if (UC_USE_WAYPOINTS) {
                    oArea = UC_GetDestinationArea(oPC, (sDestAreaX + sCurAreaY + sCurAreaZ));
                } else {
...
/*added by UC*/ if (UC_USE_WAYPOINTS) {
            ...
            case 5:  // Moving NORTHWEST
            {
                ...
                {
/*added by UC*/     if (UC_USE_WAYPOINTS) {
                        oArea = UC_GetDestinationArea(oPC, (sDestAreaX + sDestAreaY + sCurAreaZ));
                    } else {
...
void TransportEffect (object oTarget, float fDuration)
{
    ...
    DelayCommand(fTransitionDelay, ApplyEffectToObject(DURATION_TYPE_TEMPORARY, eInvis, oTarget, UC_INVIS_DURATION));
...
/*added by UC*/
    object oWP = GetObjectByTag(sAreaTag); // sAreaTag is the XYZ coords tag
    object oArea = GetArea(oWP);

    if (nDEBUG) {
        SendMessageToPC(GetFirstPC(), "GetIsValidTransArea-------------");
        SendMessageToPC(GetFirstPC(), "Destination oWP is " + GetName(oWP) + ".");
        SendMessageToPC(GetFirstPC(), "oWP Tag is " + GetTag(oWP) + ".");
    }

    if (oArea != OBJECT_INVALID){
        if (nDEBUG) SendMessageToPC(GetFirstPC(), "Destination oArea is " + GetName(oArea) + ".");
        return TRUE;
    } else {
        if (nDEBUG) SendMessageToPC(GetFirstPC(), "Destination oArea is invalid.");
        return FALSE;
    }
}
...
string UC_GetCurrentAreaWPTag(object oPC)
{
    ...
    string sMessage = "[ERROR: AT waypoint does not exist in area" +
                    GetName(oArea) + " (Area Tag: " +
                    GetTag(oArea) +  ").]";
    string sWPTag, sWPName;

    if (nDEBUG) SendMessageToPC(oPC, "Current oArea name: " + GetName(oArea));

    while (GetIsObjectValid(oWP)) {
        if (GetObjectType(oWP) == OBJECT_TYPE_WAYPOINT) {
            sWPTag = GetTag(oWP);
            sWPName = GetStringUpperCase(GetName(oWP));

            if (nDEBUG) {
                SendMessageToPC(oPC, "sWPName: " + sWPName);
                SendMessageToPC(oPC, "sWPTag: " + sWPTag);
            }
            if (GetStringLeft(sWPName, 3) == UC_AT_WP_PREFIX) {
                if (nDEBUG) SendMessageToPC(oPC, "UC_AT_WP_PREFIX " + UC_AT_WP_PREFIX + " found.");
                break;
            }
        }
    ...
    if (!GetIsObjectValid(oWP)) {
        // send message to all indicating no such WP found
...
object UC_GetDestinationArea(object oPC, string sCoordsTag)
{
    object oWP = GetObjectByTag(sCoordsTag);
    object oCurrentArea = GetArea(oPC);
    object oDestinationArea;
    string sMessage = "[ERROR: Destination waypoint tag " + sCoordsTag +
                    " not found while travelling from area " +
                    GetName(oCurrentArea) + " (Area Tag: " +
                    GetTag(oCurrentArea) +  ").]";

    if ( GetIsObjectValid(oWP) && ((GetObjectType(oWP) == OBJECT_TYPE_WAYPOINT)) ) {
        oDestinationArea = GetArea(oWP);
        if (nDEBUG) {
            SendMessageToPC(oPC, "oWP found----------------");
            SendMessageToPC(oPC, "Name of oWP " + GetName(oWP));
            SendMessageToPC(oPC, "Name of oWP " + GetTag(oWP));
            SendMessageToPC(oPC, "Name of oDestinationArea " + GetName(oDestinationArea));
            SendMessageToPC(oPC, "Tag of oDestinationArea " + GetTag(oDestinationArea));
        }
    } else {
        // send message to all indicating no such WP found
        SendMessageToPC(oPC, sMessage);
        WriteTimestampedLogEntry(sMessage);
        oDestinationArea = GetArea(oPC);
    }
    return oDestinationArea;
}
