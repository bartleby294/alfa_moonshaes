...
void hcDisabledSetup(object oPlayer)
{
    ...
    if(GetLocalInt(oMod,
        "PlayerDisabled"+GetName(oPlayer)+GetPCPublicCDKey(oPlayer)) == FALSE) {
    ...
    SetLocalInt(oMod,"PlayerDisabled"+GetName(oPlayer)+GetPCPublicCDKey(oPlayer), 1 );
}
...
void hcDisabledRemove(object oVictim)
{
    ...
    SetLocalInt(oMod,"PlayerDisabled"+GetName(oVictim)+GetPCPublicCDKey(oVictim), 0);
}
