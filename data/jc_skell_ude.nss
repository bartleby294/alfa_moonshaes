...
void KillTroll( object oTroll = OBJECT_SELF )
{
    ...
    if( GetLocalInt( oTroll, "bAcid" ))
        eAcid = EffectVisualEffect( VFX_IMP_HOLY_AID );
...
int CalculateTrollDamage()
{
    ...
    iAcidDamage = GetDamageDealtByType( DAMAGE_TYPE_DIVINE );
    if( iAcidDamage > 0)
    ...
    {
      effect eHeal = EffectHeal(GetDamageDealtByType( DAMAGE_TYPE_NEGATIVE ) * 3);
...
void HealTroll()
{
    ...
    // temporary damage.
    iHeal = ( iTempDamage < 1 ) ? iTempDamage : 1;
    iTempDamage -= iHeal;
