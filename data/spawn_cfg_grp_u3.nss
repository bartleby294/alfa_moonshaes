...
/*
        // acquire the specific Group Merchant object
        oNESSGroupMerchant = GetObjectByTag(sGroupMerchantTag);

        if (CAMP_GROUP_DEBUG) {
            SendMessageToPC(GetFirstPC(), "Group Merchant Tag: " + GetTag(oNESSGroupMerchant));
            SendMessageToPC(GetFirstPC(), "Group sGroupMerchantTag: " + sGroupMerchantTag);
        }
        // check that the Group Merchant is a valid object
        if (GetIsObjectValid(oNESSGroupMerchant))  {

            if (CAMP_GROUP_DEBUG) SendMessageToPC(GetFirstPC(), "NESS Group Merchant is valid.");

            // count Items in Group Merchant's inventory
            oItem = GetFirstItemInInventory(oNESSGroupMerchant);
            // loop through all the item object's inthe Group Merchant's inventory
            while (GetIsObjectValid(oItem)) {
                nCount++ ; // keep count of the number of items

                if (CAMP_GROUP_DEBUG) SendMessageToPC(GetFirstPC(), "Group Merchant Item Search " + IntToString(nCount) +" is: " + GetTag(oItem));

                oItem = GetNextItemInInventory(oNESSGroupMerchant);
            }

            if (CAMP_GROUP_DEBUG) SendMessageToPC(GetFirstPC(), "NESS Group Merchant has " + IntToString(nCount) + " items.");

            // randomly select an item from the inventory
            nRandom = Random(nCount) + 1;

            if (CAMP_GROUP_DEBUG) SendMessageToPC(GetFirstPC(), "Random item number " + IntToString(nRandom) + " has been selected." );

            // Get the randomly selected item
            oItem = GetFirstItemInInventory(oNESSGroupMerchant);
            // check that the random item is not the first item in the inventory
            if (nRandom > 1) {
                 // loop through all the inventory items until the random number
                 // selected has been found
                 for (i = 2; i < nRandom; i++) {
                    if (CAMP_GROUP_DEBUG) SendMessageToPC(GetFirstPC(), "i = " + IntToString(i));
                    oItem = GetNextItemInInventory(oNESSGroupMerchant);
                }
            ...
            // get the item's Tag; this is the randomly spawned creature's ResRef)
            sCreatureResRef = GetTag(oItem);
/*
            if (CAMP_GROUP_DEBUG) SendMessageToPC(GetFirstPC(), "The chosen creature ResRefis " + sCreatureResRef);

        } else {

            if (CAMP_GROUP_DEBUG) SendMessageToPC(GetFirstPC(), "Could not find NESS Spawn Merchant ["
                            + GetName(oNESSGroupMerchant) + " ] for Group: " + sTemplate);

            // Write to log
            PrintString("Could not find NESS Spawn Merchant ["
                + GetName(oNESSGroupMerchant) + " ] for Group: " + sTemplate);
        }
    }
*/
