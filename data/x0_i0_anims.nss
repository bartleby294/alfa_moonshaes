...
//:: Modified By: Joseph Van Riper
//:: Modified On: 01/17/2005
...
// Causes two creatures to face each other.
// This simply figures out where each creature is, and
// adjusts the facing towards each other.
void AnimActionFaceEachOther( object oMe, object oFriend );
...
void AnimActionStartTalking(object oFriend, int nHDiff=0)
{
        ...
        || GetAnimationCondition(NW_ANIM_FLAG_IS_MOBILE_CLOSE_RANGE))
    {
        ...
        AnimActionFaceEachOther( oMe, oFriend );
...
void AnimActionGoThroughDoor(object oDoor)
{
    ...
    ActionDoCommand(SetLocalInt(oDoor, "BEING_CLOSED", TRUE));
    ActionDoCommand(SetAnimationCondition( NW_ANIM_FLAG_IS_INTERACTING ));
    ...
    ActionDoCommand(SetAnimationCondition( NW_ANIM_FLAG_IS_INTERACTING, FALSE ));
}
...
int AnimActionGoInside()
{
    ...
    if ( !GetLocalInt( GetArea(OBJECT_SELF), "NW_DOORS_INIT" ) )
    {
        // Iterate through all the doors, looking for those that
        // lead to taverns or shops.
        // Only want to do this once for the area.
        int nIter = 1;
        int nMax = 0;
        object oDoor = GetNearestObject( OBJECT_TYPE_DOOR );
        while ( GetIsObjectValid(oDoor) )
        {
            ...
            if ( GetIsObjectValid(oDest) )
            {
                ...
                if ( GetIsObjectValid(oWay) )
                {
                    // We've found a door leading to a tavern or shop.
                    // We'll make it easy to find this door later.
                    nMax = nMax + 1;
                    SetLocalObject( GetArea(OBJECT_SELF), "NW_DOOR_INIT_" + IntToString( nMax ), oDoor );
                }
            }
            nIter = nIter + 1;
            oDoor = GetNearestObject( OBJECT_TYPE_DOOR, OBJECT_SELF, nIter );
        }
        SetLocalInt( GetArea(OBJECT_SELF), "NW_DOORS_INIT_MAX", nMax );
        SetLocalInt( GetArea(OBJECT_SELF), "NW_DOORS_INIT", TRUE );
    }

    int nMax = GetLocalInt( GetArea(OBJECT_SELF), "NW_DOORS_INIT_MAX" );
    if ( nMax > 0 )
    {
        nMax = Random(nMax - 1) + 1;
    }

    object oDoor = GetLocalObject( GetArea(OBJECT_SELF), "NW_DOOR_INIT_" + IntToString( nMax ) );
