...
void SetGlobalIgnore(){
  ...
  SetLocalInt(oMod, "IgnoreShouts", 1);
  DelayCommand(0.5, SetLocalInt(oMod, "IgnoreShouts", 0));

}

int GetGlobalIgnore(){
  object oMod = GetModule();
  return GetLocalInt(oMod, "IgnoreShouts");
...
void main()
{
    ...
    if(GetLocalTimer(AI_TIMER_SHOUT_IGNORE_ANYTHING_SAID) == TRUE){
      return;
    }
    ...
    if(oShouter == OBJECT_SELF){
      return;

    }


    if(GetIgnore(oShouter)){
      return;

    }

    if(GetIsInCombat(OBJECT_SELF)){
      //DetermineCombatRound(GetLastHostileActor(OBJECT_SELF));
      return;
    ...
    //SendMessageToAllDMs(GetName(OBJECT_SELF));
        ...
        GetArea(oShouter) != GetArea(OBJECT_SELF))/* Same area (Stops loud yellow shouts getting NPCs) */
    {
        ...
        //SendMessageToAllDMs("Conversation Script: I heard something!");
    ...
    {
        //SendMessageToAllDMs("Conversation Script: Not ignored target!");
        if(GetIsFriend(oShouter) || GetFactionEqual(oShouter))
        {
              ...
              !GetIsPC(oShouter) && !GetIsPC(GetMaster(oShouter)))
            {
                ...
                //SendMessageToAllDMs("Conversation Script: There was a match, not a PC!");
                if(GetIsInCombat(oShouter) ){
                  if( GetObjectHeard(GetLastHostileActor(oShouter)) || GetObjectSeen(GetLastHostileActor(oShouter)) ){
                    ClearAllActions();
                  ...
                  ActionMoveToLocation(GetLocation(GetLastHostileActor(oShouter)));
                  SetLocalTimer(AI_TIMER_SHOUT_IGNORE_ANYTHING_SAID, 6.0);
                  SetGlobalIgnore();
                  //SendMessageToAllDMs("Conversation Script: Now I'm moving in to where attack came from.");
                  }
                }
                else{
                  //ClearAllActions();
                  //DetermineCombatRound(GetLastHostileActor(oShouter));
                  //ActionMoveToLocation(GetLocation(oShouter));
                  //SendMessageToAllDMs("Conversation Script: He's not in combat!  Don't respond.");
                }
                     ...
                     GetObjectType(oShouter) == OBJECT_TYPE_CREATURE)
            {
                ...
                if(!GetIsInCombat() ){
                 if( GetObjectHeard(GetLastHostileActor(oShouter)) || GetObjectSeen(GetLastHostileActor(oShouter)) ){
                  ...
                  //SendMessageToAllDMs("Conversation Script: I can't determine combat round.  So move to shouter");
                }
                else{
                  //SendMessageToAllDMs("Conversation Script: Not sure what to do.");
                  ActionMoveToLocation(GetLocation(GetLastHostileActor(oShouter)));
                  SetLocalTimer(AI_TIMER_SHOUT_IGNORE_ANYTHING_SAID, 6.0);
                  SetGlobalIgnore();

                }
                //SendMessageToAllDMs("Conversation Script: My friend is in combat!");
                // Respond to oShouter
        ...
        else if(GetIsEnemy(oShouter) && GetObjectType(oShouter) == OBJECT_TYPE_CREATURE)
        {
            ...
            //SendMessageToAllDMs("Conversation Script: Heard an enemy!");
    ...
    // Fire End of Dialogue event
    //SendMessageToAllDMs("End: " + GetName(OBJECT_SELF));
